...
1. Define the GPR survey in Python using `elfe3d_gpr.inputs.survey.GPRSurvey.build()`.
2. Generate the mesh input and field input files with `survey.generate()`.
3. Run TetGen on the generated `.poly` file using `elfe3d_gpr.runner.run_tetgen()`.
4. Execute the Fortran solver with `elfe3d_gpr.runner.run_solver()`.
...
- `elfe3d_gpr.runner` contains the execution layer and host path handling.
- `elfe3d_gpr.inputs.survey` is the central model builder.
- `elfe3d_gpr.inputs.writeinputfiles` writes the solver input files.
- `elfe3d_gpr.inputs.writetetgenpoly` writes the TetGen geometry input.
...
| Field | Description |
|---|---|
| `air_eps_r` | Relative permittivity for the air layer |
| `layer_sigma` | Electrical conductivity for each earth layer |
