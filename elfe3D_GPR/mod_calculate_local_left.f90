...
contains
  ...
  subroutine calc_stiffness_matrix (l, el2edl, Ve, ed_sign, &
    ...
    K = cmplx(0.0_dp, 0.0_dp, kind=dp)

    do concurrent (j = 1:6, i = 1:6)

          K(i,j) = (1.0_dp/mu_r(l)) * ((4.0_dp*el2edl(l,i)*el2edl(l,j)*ve(l))/&
                    ((6.0_dp*ve(l))**4.0_dp))*&
               (&
  ...
  subroutine calc_mass_matrix (l, el2edl, Ve, ed_sign, &
    ...
    do concurrent (i = 1:6, j = 1:6)
       MM(i,j) = (k_0**2 * epsilon_r_eff(l)) * MM(i,j) * ed_sign(l,i)*ed_sign(l,j)
    end do
  ...
  end subroutine calc_stiffness_matrix_anisotropic_PML
  ...
  subroutine calc_mass_matrix_anisotropic_PML (l, el2edl, Ve, ed_sign, &
    ...
    do j = 1,4
       do i = 1,4
          ...
          f(i,j) = b(l,i)*b(l,j)*LAMBDA_l(1) + c(l,i)*c(l,j)*LAMBDA_l(2) + d(l,i)*d(l,j)*LAMBDA_l(3)
  ...
  end subroutine calc_mass_matrix_anisotropic_PML
