...
program elfe3d
  ...
  real(kind=dp), allocatable, dimension(:) :: rho, mu, epsilon
  ...
  call read_model_param(eleattr, M, rho, mu, epsilon)
  ...
  frequency_loop: do numfreq = 1,Nfreq
     ...
     matrix_element_loop: do l = 1,M
        ...
        AA = (0.0_dp, 0.0_dp)
        AA = cmplx((1/mu(l))*K - ((w**2)*(epsilon(l)))*MM, &
                    -w*(1/rho(l))*MM, kind=dp)
