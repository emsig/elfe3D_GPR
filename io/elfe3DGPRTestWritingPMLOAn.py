...
class elfe3DGPRTestWritingPML:
    ...
    def write_base_input_file(self):
        ...
        with open(self.input_base_file, 'w+') as f:
            ...
            f.write("{}\t{}\t{}\n".format(self.x_min-PML_BUFF, self.y_min-PML_BUFF, self.z_min-PML_BUFF))
            f.write("{}\t{}\t{}\n".format(self.x_max+PML_BUFF, self.y_max+PML_BUFF, self.z_max+PML_BUFF))
    ...
    def evaluate_all_nodes(self):
        ...
        # Nodes for the anomaly box
        self.node_list_anomaly = []
        nodes_an, self.num_node = self.create_rectangular_prism_nodes(
    ...
    def evaluate_all_facets(self):
        ...
        # Anomaly Facet
        an_faces = self.create_cuboid_faces_from_nodes(self.node_list_anomaly)
        self.anomaly_string = "\n# anomaly facet\n"
    ...
    def evaluate_all_regions(self):
        ...
        # Anomaly Region
        self.num_regions += 1
        an_vol = (float(3e8/(self.f_list[0]*np.sqrt(self.a_mat[0])))/20)**3 / (6 * np.sqrt(2))
        self.regions.append([
            self.num_regions, 0, 0, (self.az[0]+self.az[1])/2, self.num_regions, f"{an_vol:.4e}", "# Anomaly Region",
            1/self.a_mat[1], self.a_mat[2], self.a_mat[0]
            ])
