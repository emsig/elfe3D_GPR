...
% Super Title
sgtitle(['Reflection response, d = ', num2str(d,2), ' cm, \epsilon_{r;1}=', num2str(epsr1,2), ', \epsilon_{r;2}=', num2str(epsr2,2)], 'FontSize', 20, 'FontWeight', 'bold')
...
for ip = 1:3
  ...
  abs_exx = abs(exx(ip, :));
  phase_exx = angle(exx(ip, :)) * 180 / pi; % Convert phase to degrees

  % Combine data into the matrix
  output_data(:, 2 + (ip - 1) * 4) = real(exx(ip, :)); % Real part
  output_data(:, 3 + (ip - 1) * 4) = imag(exx(ip, :)); % Imaginary part
  output_data(:, 4 + (ip - 1) * 4) = abs_exx; % Magnitude
  output_data(:, 5 + (ip - 1) * 4) = phase_exx; % Phase
end

% Write to CSV file
csv_filename = sprintf('Exx_output_r%d.csv', round(r));
header = {'Frequency', 'Real_Broadside', 'Imag_Broadside', 'Abs_Broadside', 'Phase_Broadside', ...
      'Real_45deg', 'Imag_45deg', 'Abs_45deg', 'Phase_45deg', ...
      'Real_Endfire', 'Imag_Endfire', 'Abs_Endfire', 'Phase_Endfire'};
